-- LIVE schema of public schema — read from the database 2026-09-23T01:26:55.318Z
...
-- visit_products           table (17 cols)
-- visits                   table (14 cols)
...
--   constraint customers_tier_check [c] CHECK (((tier IS NULL) OR (tier = ANY (ARRAY['Silver'::text, 'Gold'::text]))))
--   index CREATE INDEX customers_area_idx ON public.customers USING btree (area)
...
--   index CREATE INDEX visit_products_bill_idx ON public.visit_products USING btree (bill_number) WHERE (bill_number IS NOT NULL)
--   index CREATE INDEX visit_products_drop_subcategory_idx ON public.visit_products USING btree (visit_id) WHERE (drop_subcategory IS NOT NULL)
--   index CREATE UNIQUE INDEX visit_products_pkey ON public.visit_products USING btree (id)
--   index CREATE INDEX visit_products_staff_note_idx ON public.visit_products USING btree (visit_id) WHERE (staff_note IS NOT NULL)
...
--   index CREATE INDEX visits_suite_idx ON public.visits USING btree (store_id, suite) WHERE (suite IS NOT NULL)
...
-- visit_products :: visit_products_bill_idx
-- visit_products :: visit_products_drop_subcategory_idx
-- visit_products :: visit_products_pkey
-- visit_products :: visit_products_staff_note_idx
