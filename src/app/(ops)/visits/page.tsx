...
function VisitCard({ v }: { v: VisitLive }) {
  ...
  const label = active ? "Fitting in progress" : v.customerId ? "Ready to start" : "Needs identification";
  const name = v.customerName || "Unidentified customer";
  const unassigned = !v.assignedSalespersonId;
  ...
  return (
    <li className="flex flex-col rounded-2xl border border-[var(--fp-line)] bg-[var(--fp-surface)] p-5 shadow-[var(--fp-shadow)]">
      ...
      {unassigned && v.customerId && (
        <div className="mt-4">
          <p className="mb-1.5 text-[12px] font-semibold uppercase tracking-[0.12em] text-[var(--fp-faint)]">Assign FC</p>
      ...
      <div className="mt-5 flex flex-wrap items-center gap-2">
        <Link
          href={`/visits/${v.id}`}
          className={`inline-flex min-h-12 flex-1 items-center justify-center rounded-xl px-5 text-[15px] font-semibold text-white ${active ? "bg-[var(--fp-ink)] hover:bg-black" : "bg-[var(--fp-brand)] hover:bg-[var(--fp-brand-deep)]"}`}
        >
          {active ? "Continue visit" : "Start visit"}
        </Link>
