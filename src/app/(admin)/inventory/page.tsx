...
import { SEED_PRODUCTS, SEED_CATEGORIES, stockStatus } from "@/lib/inventory";
...
function InventoryInner() {
  ...
  const { page, setPage } = usePageParam(`${q}|${cat}|${stock}`);

  const rows = useMemo(() => SEED_PRODUCTS.filter((p) => {
    if (cat !== "all" && p.categoryId !== cat) return false;
    if (stock !== "all" && stockStatus(p) !== stock) return false;
    if (q.trim()) {
      const needle = q.trim().toLowerCase();
      if (!`${p.name} ${p.sku} ${p.supplierName} ${p.barcode ?? ""} ${p.companyBarcode ?? ""} ${p.brandName ?? ""} ${p.designNo ?? ""}`.toLowerCase().includes(needle)) return false;
    }
    return true;
  }), [q, cat, stock]);

  const paged = paginate(rows, page, DEFAULT_PAGE_SIZE);
  ...
  return (
    <div className="staff-page">
      <PageHeader
        kicker="Catalogue"
        title="Inventory"
        sub={`${rows.length} of ${SEED_PRODUCTS.length} SKUs · search, filter, open for stock moves.`}
      ...
      <Card className="shadow-[0_1px_2px_rgba(28,25,23,0.04)]">
        <CardContent className="flex flex-col gap-2 pt-4 md:flex-row">
          <div className="relative flex-1">
            ...
            <Input value={q} onChange={(e) => setQ(e.target.value)} placeholder="Search name, SKU, barcode, brand, design…" className="min-h-[48px] rounded-xl pl-10 transition-all focus:ring-4 focus:ring-[#b4234d]/10" aria-label="Search inventory" />
            {q && (
          ...
          <div className="grid grid-cols-2 gap-2 md:flex md:gap-2">
            <Select value={cat} onValueChange={setCat}>
              ...
              <SelectContent>
                ...
                {SEED_CATEGORIES.map((c) => <SelectItem key={c.id} value={c.id}>{c.name}</SelectItem>)}
              </SelectContent>
            </Select>
        ...
        <CardContent className="flex flex-col divide-y p-0">
          ...
          {!rows.length && (
            <div className="flex flex-col items-center gap-1.5 px-6 py-12 text-center">
              ...
              <button onClick={() => { setQ(""); setCat("all"); setStock("all"); }} className="mt-2 inline-flex min-h-[44px] items-center rounded-xl border px-4 text-[13.5px] font-semibold transition-all hover:-translate-y-px hover:border-foreground">Clear filters</button>
            </div>
          )}
        </CardContent>
      </Card>
